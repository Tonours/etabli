...
function usage(exitCode = 0) {
  ...
  output.write(
    "Usage: runtime-skill-canary [--skill name] [--repo path] [--home path] [--runtime list] [--timeout seconds] [--live] [--json]\n",
  );
    ...
    switch (arg) {
      ...
      case "--runtime":
        ...
        if (
          ...
          options.runtimes.some((value) => !RUNTIMES.includes(value))
        ) {
...
function invokeRuntime(runtime, options, expected) {
  ...
  const prompt =
    "Invoke the shared runtime-skill-canary skill and reply with only its documented canary response. Do not use tools.";
  ...
  if (result.status !== 0)
    return { state: "unknown", reason: `exit_${result.status}` };
  const output = `${result.stdout || ""}\n${result.stderr || ""}`;
  if (!output.split(/\s+/).includes(expected))
    return { state: "failed", reason: "canary_response_missing" };
...
function main() {
  ...
  if (sourceExists) {
    const match = readFileSync(skillPath, "utf8").match(
      /^Canary response: `([A-Z0-9_]+)`$/m,
    );
    canaryResponse = match?.[1] || null;
  }
  const liveAuthorized =
    options.live && process.env.RUN_SKILL_RUNTIME_CANARY === "1";
  const runtimes = {};
  for (const runtime of options.runtimes) {
    if (!options.live)
      runtimes[runtime] = { state: "skipped", reason: "live_not_requested" };
    else if (!liveAuthorized)
      runtimes[runtime] = { state: "skipped", reason: "live_opt_in_missing" };
    else if (!sourceLocked || !linkValid || !canaryResponse)
      runtimes[runtime] = {
        state: "skipped",
        reason: "offline_precondition_failed",
      };
