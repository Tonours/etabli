...
- you need resource-level authorization patterns more than route-tree structure —
  use `tanstack-start-resource-ownership` instead
- the task is about general routing patterns (dynamic routes, structure
  progression, route-vs-component heuristics) rather than access boundaries —
  use `tanstack-start-route-patterns` instead
