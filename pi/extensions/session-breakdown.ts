 ...
 * /session-breakdown
 *
 * Local override for mitsupi's session-breakdown extension.
 * Keep this until upstream stops importing non-exported pi-tui helpers.
 *
...
function sliceByColumn(text: string, start: number, width: number, fromEnd = false): string {
	if (width <= 0) return "";
	const ansiPattern = /\x1b\[[0-9;]*m/g;
	const ansiParts: Array<{ index: number; value: string }> = [];
	let plain = "";
	let plainIndex = 0;
	...
	while ((match = ansiPattern.exec(text))) {
		...
		ansiParts.push({ index: plainIndex, value: match[0] });
		lastIndex = match.index + match[0].length;
	}
	plain += text.slice(lastIndex);

	const chars = Array.from(plain);
	const sliceStart = Math.max(0, Math.min(chars.length, start));
	const sliceEnd = Math.max(sliceStart, Math.min(chars.length, sliceStart + width));
	const activeAnsi = ansiParts.filter((part) => part.index <= sliceStart).map((part) => part.value).join("");
	let result = activeAnsi;

	for (let i = sliceStart; i < sliceEnd; i++) {
		result += chars[i];
		for (const part of ansiParts) {
			if (part.index === i + 1) result += part.value;
		}
	}

	if (fromEnd && !result.endsWith("\x1b[0m") && activeAnsi) {
		result += "\x1b[0m";
	}
