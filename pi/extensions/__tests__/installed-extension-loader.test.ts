...
const piLauncher = execFileSync("which", ["pi"], { encoding: "utf8" }).trim();
...
const activePiVersion = launcherTrace.stdout.trim();
...
function loadWithNode(paths: string[]) {
...
describe("installed Pi extension loader", () => {
	test("reproduces the logical-path failure and loads every extension with the managed layout", () => {
		...
		try {
			...
			const baselineResult = loadWithNode([join(baseline, "workflow-router.ts"), join(baseline, "workflow-run-binding.ts")]);
			...
			const result = loadWithNode(installedPaths);
