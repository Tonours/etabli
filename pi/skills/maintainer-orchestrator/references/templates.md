...
Rules:
- Do not create subworkers, delegate work, or manage other workers.
...
Use this only when the user asks to keep monitoring or continue later, with the host's recurring-loop primitive (for example a scheduled loop).

```text
Use $maintainer-orchestrator to continue this orchestration.

Before acting:
- Read the latest worker states and any local ledger/memory.
- Do not create, relabel, stop, or steer workers before reading their latest state.
