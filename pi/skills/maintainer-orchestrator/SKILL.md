...
description: Coordinate maintenance across local/GitHub repositories, issue and PR queues, CI/review gates, repo-scoped workers, and owner decision briefs. Use when the user asks the agent to maintain repos, triage a portfolio, run autonomous repo work, wake up on a cadence, monitor workers, prepare merge/close/release decisions, or orchestrate multiple repository tasks without losing authorization boundaries.
---

# Maintainer Orchestrator

Coordinate repository work as a control plane. Keep this orchestration lightweight: inspect, classify, delegate only when authorized, monitor gates, ask decision-ready questions, and report. Put deep repository investigation, implementation, review, CI repair, and live proof in repo-scoped workers or the repo's normal workflow.
...
Separate observed facts from assumptions. If the target is ambiguous, ask one narrow question before creating workers or mutating anything.
...
- Delegation/worker creation requires explicit authorization.
...
Use the host harness's subagent or task tools (for example Pi Agent/Task* primitives or Claude subagents) when available and explicitly authorized. This root orchestrator owns the worker lifecycle: spawning, reuse, labeling, steering, completion, and handoff. Workers must not create subworkers or manage other workers.

Delegation rules:

- One worker lane per repo unless a repo has clearly disjoint authorized work.
- Prefer an isolated worktree or dedicated worker for code edits and a lightweight read-only worker for triage.
- Reuse the existing repo worker when it has the same repo and no conflicting active task.
- Label worker tasks when assigning or materially changing work: `<Project>: <short current task>`.
...
- Do not interrupt, relabel, stop, or replace a worker before reading its latest state.

Read `references/templates.md` before drafting worker prompts, owner decision briefs, recurring follow-up prompts, or ledger entries.

## Monitoring

Before sending any worker message:

1. Read the worker's latest state, newest user/delegation instructions, run status, and outputs when needed.
2. Treat newest worker-local instruction as authoritative for that worker.
...
For recurring follow-up, use the host's recurring-loop primitive (for example a scheduled loop) to continue this orchestration on an interval. Do not create a recurring routine unless the user asks to keep monitoring. Read and update the ledger/memory when present.
...
Keep a compact ledger when work spans repos, workers, or sessions. Default path: `~/.pi/agent/maintainer-orchestrator.md` unless the repo has a more appropriate handoff location. The orchestrator owns the ledger; workers do not edit it.
