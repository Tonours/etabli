...
Analyze a Linear bug with maximum certainty. This is read-only: no file edits,
no implementation, no Linear comments.
...
cannot be read, stop with `LINEAR_MCP_UNAVAILABLE` or `LINEAR_ISSUE_NOT_FOUND`.
...
2. Analyze impacted code:
   ...
   - use `rg` and focused file reads to locate candidate files
   - read the full functions/classes involved, not isolated snippets
   - trace execution from input to observed output
   - list each root-cause candidate with `file:line`, mechanism, and 3-10 lines
     of relevant code
...
4. Verdict:
   ...
   - `HIGH CONFIDENCE`: root cause very likely but 1-2 alternatives remain
