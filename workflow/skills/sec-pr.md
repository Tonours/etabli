...
Shared contract for auditing Dependabot or security GitHub PRs.

Runtime adapters may add tool syntax or source-resolution details. They must not
change the read-only default, independent-evidence requirement, lockfile
verification rule, or `PASS | FAIL | INVESTIGATE` verdict model.
...
explicitly asked for post-validation actions and the verdict is `PASS`. Never
merge automatically.
...
2. Independent truth:
   ...
   - fetch GHSA advisory: `gh api advisories/<GHSA_ID>`;
...
3. Isolated worktree, per `workflow/skills/worktree-isolation.md`:
   ...
   - judge install success by exit code and populated `node_modules`, not by the
     word `error` in logs;
   - if install truly fails, use static lockfile verification and mark that
     fact;
   - always remove the worktree and temp marker on exit.
4. Fixed:
   - confirm package.json dependency or resolution is at least patched;
   - confirm lockfile resolved version is at least patched;
