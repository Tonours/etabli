...
## Périmètre
Repos ForestAdmin : `forestadmin`, `forestadmin-server`, `agent-nodejs`, `forest-for-zendesk`. Fenêtre : aujourd'hui (calcule la date via Bash `date -u +%Y-%m-%d`).

## Collecte (gh CLI)
1. PRs mergées aujourd'hui sur chaque repo : `gh search prs --merged --owner=ForestAdmin --merged-at=">=<date>" --json repository,number,title,author,url` (ou par repo). Pour chacune : titre, auteur, fichiers touchés (`gh pr view <n> -R <repo> --json files` si besoin).
2. Les PRs OUVERTES de `Tonours` et leurs fichiers, pour le croisement : `gh search prs --author=Tonours --state=open --owner=ForestAdmin --json repository,number,url`.
...
- elle touche ses zones connues : `workflow-visualizer`, `workflow-editor`, module federation / MFE, le package standalone workflow dans forestadmin ; `packages/agent` dans agent-nodejs ; n'importe quoi dans forest-for-zendesk.
