...
## Périmètre
Quatre repos de l'organisation ForestAdmin UNIQUEMENT : `forestadmin`, `forestadmin-server`, `agent-nodejs`, `forest-for-zendesk`. Deux populations :
...
- Population A : `gh search prs --author=Tonours --state=open --owner=ForestAdmin --created=">=YYYY-MM-DD" --json repository,number,title,url,createdAt`
- Population B : `gh search prs --review-requested=Tonours --state=open --owner=ForestAdmin --json repository,number,title,url`
- Détail CI/reviews/mergeable : `gh pr view <n> -R ForestAdmin/<repo> --json mergeable,reviewDecision,statusCheckRollup,comments,reviews` et `gh pr checks <n> -R ForestAdmin/<repo>`.
