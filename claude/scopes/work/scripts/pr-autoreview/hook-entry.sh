...
case "$pr_autoreview_remote_url" in
  ...
  *ForestAdmin/forestadmin-server*|\
  *ForestAdmin/forestadmin.git*|\
  *ForestAdmin/forestadmin|\
  *ForestAdmin/forest-for-zendesk*) ;;
  *) return 0 ;;
esac

pr_autoreview_repo=$(printf '%s' "$pr_autoreview_remote_url" | sed -e 's,.*ForestAdmin/,,' -e 's,\.git$,,')
