# Profil de review — employer-server (employer/employer-server)
...
- **Imports internes par la racine du package**, jamais `dist`/`src` (`no-restricted-imports` sur `@employer/*`).
...
2. **Import de `bluebird` ou import direct depuis `dist`/`src` d'un `@employer/*`** — interdit par ESLint, mais un cache lint non invalidé peut le laisser passer.
