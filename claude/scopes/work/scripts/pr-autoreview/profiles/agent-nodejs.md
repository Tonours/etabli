# Profil de review — agent-nodejs (ForestAdmin/agent-nodejs)
...
- Test : **Jest** (`package.json:49`), config `jest.config.ts:11` : `testMatch: ['<rootDir>/packages/*/test/**/*.test.ts']` — tests **non colocalisés**, sous `test/` en miroir de `src/`. Setup global `jest-extended/all` (`jest.config.ts:12`).
  ...
  - Par package : `yarn workspace @forestadmin/<pkg> test -- <path ou -t "nom">`.
...
- `agent` — point d'entrée SDK, `createAgent()`, orchestre HTTP + customizer + services. Dépend de `datasource-customizer`, `datasource-toolkit`, `forestadmin-client`, `mcp-server` (`packages/agent/package.json:16-20`).
...
- `forestadmin-client` — SDK vers l'API SaaS Forest : permissions, scopes, OIDC, schema push, activity logs. Toute la couche HTTP passe par `ServerUtils`.
...
- `plugin-aws-s3`, `plugin-export-advanced`, `plugin-flattener`, `forest-cloud`, `agent-toolkit`, `agent-testing`, `_example`.
...
2. **Casing operators snake_case vs PascalCase** — `packages/agent/src/routes/capabilities.ts:60-67,87-92` convertit en snake_case, `packages/mcp-server/src/schemas/filter.ts:3-48` attend un enum PascalCase (44 operators). Un changement d'un côté sans l'autre casse le filtrage MCP silencieusement. (kb `forest-capabilities-operators`)
...
4. **Collision de slug d'action gatée par un flag** — `packages/agent/src/utils/forest-schema/generator-collection.ts:47-72` : `assertNoActionSlugCollision` ne tourne que si `useUnsafeActionEndpoint` est actif. (kb `action-slug-collision`)
...
7. **workflow-executor, config** : jamais de `process.env` hors `cli-core`/`tracing.ts`. Régression déjà survenue (`FOREST_EXECUTOR_ENCRYPTION_KEY` lu dans `crypto/`).
