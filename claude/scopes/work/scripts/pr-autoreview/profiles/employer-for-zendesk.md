...
6. **Mapping contextField abandonné** (kb `zendesk-context-field-mapping`, ADR superseded) — `contextField` existait en v1.0.2, retiré en v1.1.0 ; le `manifest.json` ne porte plus que le seul paramètre d'URL (`manifest.json:25-33`). Le matching reste heuristique. Traiter toute PR qui réintroduit un mapping déclaratif comme une reprise de décision, pas comme un détail.
