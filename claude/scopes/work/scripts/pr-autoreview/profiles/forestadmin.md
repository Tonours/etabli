# Profil de review — forestadmin (ForestAdmin/forestadmin, frontend)
...
- **`forest-custom/dependency-rules`** en erreur (`eslint.config.mjs:97`), implémentation `eslint-custom-rules/dependency-rules.js:26-56` : interdit d'importer un service UI (`business/notification`, `services/toastr`, `services/ui/*`) depuis un `internal-business` ; interdit d'importer un `internal-business`/`internal-feature` hors du dossier `services` de sa feature.
- **`forest-custom/class-naming-based-on-path`** en erreur (`eslint.config.mjs:315-345`) : `components/[path]/component.ts` → suffixe `Component`, `services/internal-business/[name].ts` → `InternalBusinessService`, `services/feature.ts` → `FeatureService`, `app/services/business/[name]` → `BusinessService`.
...
| Zone touchée | Skill |
|---|---|
| Composant Ember (`.hbs`, component `.ts`, SCSS) | `ember-forestadmin-components` |
| Route / controller Ember | `ember-forestadmin-routes` |
| Service ou state (feature/shared/internal-business/internal-feature) | `ember-forestadmin-services-state` |
| Modèle / adapter / serializer / GraphQL | `ember-forestadmin-data` |
| Test QUnit / mirage / sinon | `ember-forestadmin-testing` |
| Lint, typecheck, nommage de classe, SCSS | `ember-forestadmin-lint-style` |
| `apps/*` ou `packages/*` (React 19, Module Federation) | `ember-forestadmin-react-mfe` |
| Déplacement `app/legacy` → structure moderne | `ember-forestadmin-migration` |
| `workflow-visualizer` | `ember-forestadmin-workflow-visualizer` |
| Emplacement architectural ambigu (feature vs shared vs legacy) | `ember-forestadmin-architecture` |
