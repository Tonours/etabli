...
Router for backend and integration work on the employer stack. Frontend Ember work
routes through `ember-employer-suite` instead.
...
`~/work/brain/ref/employer-constants.md`. Read it instead of asking.

## Route by subject

Start at `employer-bff-gateway` for anything BFF-shaped: it is the entry note for
the initiative and carries the Mode 1 / Mode 2 arbitration the table below
assumes.

| Task touches | Read first | Then |
|---|---|---|
| Permissions, RBAC, role scoping | `employer-permissions-v4`, `private-api-permissions-route` | `employer-custom-action-authorization` for record-dependent actions |
| BFF, anything | `employer-bff-gateway` | the specific row below |
| Agent auth, JWT, token minting | `employer-auth-jwt`, `bff-agent-token-no-type-claim` | `employer-oauth-provider-mcp` on the OAuth path |
...
| MCP search, filters, operators | `mcp-search-filters`, `employer-capabilities-operators` | — |
...
| Server env vars, private-api deps | `employer-service-env-vars`, `private-api-joi-15` | — |
