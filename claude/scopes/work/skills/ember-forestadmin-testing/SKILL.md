---
name: ember-forestadmin-testing
description: Test ForestAdmin Ember code with integration-first QUnit, mirage, sinon, and stable selectors matching real project conventions.
version: 0.2.0
author: ForestAdmin Team
license: Proprietary
---

# Ember ForestAdmin Testing
...
module('Integration | Component | Feature::Inbox::CreationModal', function (hooks) {
  ...
  test('it should create an inbox', async function (assert) {
    this.server.post('/forest/inboxes', () => ({ data: { id: 'new-1', type: 'inbox' } }));
...
module('Integration | ...', function (hooks) {
  ...
  test('should handle API error', async function (assert) {
    this.server.get('/forest/inboxes', () => new Response(500, {}, { errors: [{ detail: 'fail' }] }));
