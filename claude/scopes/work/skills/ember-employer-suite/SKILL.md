---
name: ember-employer-suite
description: Route Ember/employer frontend work to the correct specialized skill before coding.
version: 0.2.0
...
Use this skill first for any task in `employer/employer` or a similarly large Ember app.

## Choose the primary skill

- New feature architecture, file placement, dependency boundaries → `ember-employer-architecture`.
- Components, templates, actions, CSS, DOM modifiers → `ember-employer-components`.
- Services, state, orchestration, business logic → `ember-employer-services-state`.
- Routes, controllers, route templates, query params → `ember-employer-routes`.
- Models, adapters, serializers, store, GraphQL/API integration → `ember-employer-data`.
- Tests, fixtures, test selectors, ember-exam → `ember-employer-testing`.
- Diagnosing bugs, resolver failures, store issues, test failures → `ember-employer-debug`.
- Lint/typecheck/style failures or naming conventions → `ember-employer-lint-style`.
- Moving legacy pods code into modern structure → `ember-employer-migration`.
- React micro-frontends hosted by Ember → `ember-employer-react-mfe`.
- `app/features/workflow-visualizer` → `ember-employer-workflow-visualizer`.
