...
The MFEs live in `apps/user-settings-mfe` and `apps/notes-listing-mfe`; they mount into the Ember host via Module Federation. The dev app at `https://app.development.employer.com` proxies to the local server running the current branch.
...
From `~/work/employer-poc`, run `yarn start:all` in the background (Ember :4200 + MFEs :4001/:4002 + workspace packages, orchestrated by nx). It is slow: workspace deps build first, then Ember compiles.
...
`tabs_context_mcp` (new tab) → `navigate` to `https://app.development.employer.com` → screenshot.
