# Forest Dependabot extras for sec-pr

Load this only after `workflow/skills/sec-pr.md`. ForestAdmin-specific Dependabot
body (Fixed/Ignored/Deferred/Resolutions), isolated worktree install, and
PASS-only checkbox/approve steps. Never merge.

Audit complet d'une PR de securite ForestAdmin (generee par l'outil maison, body
...
if [ -f .nvmrc ]; then
  nvm install && nvm use           # respecte .nvmrc (cas ForestAdmin standard)
else
...
5. **Actions sortantes seulement si PASS** — cocher + approuver uniquement apres audit complet vert (Phase 10). FAIL / À INVESTIGUER : rapport, zero action sortante.
