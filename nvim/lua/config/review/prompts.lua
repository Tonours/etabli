...
local function action_instructions(action)
  ...
  if action == "review" then
    return {
      ...
      "Anchor every line: or line_range: to the Changed lines above. If unchanged context is the reason, attach the finding to the nearest changed line and explain the context in issue:.",
      ...
      "For each finding, use exactly these labels: severity:, file:, line: or line_range:, issue:, impact:, review_comment:, suggested_fix:.",
      "Use line_range: instead of line: when the inline comment spans multiple changed lines.",
      "review_comment: must be one concise inline-ready comment suitable for a GitHub-style review thread, without code fences or tables.",
...
local function batch_action_instructions(action)
  ...
  if action == "review" then
    return {
      ...
      "Anchor every line: or line_range: to that hunk's Changed lines. If unchanged context is the reason, attach the finding to the nearest changed line and explain the context in issue:.",
      ...
      "For each finding, use exactly these labels: severity:, file:, hunk:, line: or line_range:, issue:, impact:, review_comment:, suggested_fix:.",
      "Use line_range: instead of line: when the inline comment spans multiple changed lines.",
      "review_comment: must be one concise inline-ready comment suitable for a GitHub-style review thread, without code fences or tables.",
      "Then add open questions or assumptions only when they change the decision.",
      "If you find no actionable issue, put exactly `No findings.` as the only finding, then still include the final verdict.",
