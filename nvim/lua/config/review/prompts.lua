...
local function action_instructions(action)
  ...
  if action == "review" then
    return {
      ...
      "For each finding, use exactly these labels: severity:, file:, line:, issue:, impact:, review_comment:, suggested_fix:.",
...
local function batch_action_instructions(action)
  ...
  if action == "review" then
    return {
      ...
      "For each finding, use exactly these labels: severity:, file:, hunk:, line:, issue:, impact:, review_comment:, suggested_fix:.",
      "review_comment: must be one concise inline-ready comment suitable for a GitHub-style review thread, without code fences or tables.",
      "Then add open questions or assumptions only when they change the decision.",
      "If you find no actionable issue, put exactly `No findings.` as the only finding, then still include the final verdict.",
      "If human arbitration is needed, include human_checkpoint: yes and the reason.",
