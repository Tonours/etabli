...
Function middleware can use `.inputValidator(...)`, `.client(...)`, and `.server(...)`. TypeScript enforces method order for inference.
...
Use `src/start.ts` with `createStart(...)` when a concern should apply everywhere.
