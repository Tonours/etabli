...
Get the required keys from Clerk.

### 2. Store env vars correctly
- client-safe publishable key → `VITE_*` if required by the integration path
- secret keys → server-only env vars
- never expose secret keys in client bundles

### 3. Add the provider in the app shell
Integrate Clerk at the app shell/root level where it belongs.
Do not spread provider setup across random feature files.

### 4. Create public and protected route trees
Separate:
- public pages
- authenticated pages
...
### 5. Guard sensitive server functions
...
- store the publishable/browser-safe key in a client-safe env variable when the integration path requires it
- keep secret keys server-only
- place the Clerk provider in the app shell/root level, not scattered across feature files
