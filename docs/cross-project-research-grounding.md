...
| OpenAI agent evals, https://developers.openai.com/api/docs/guides/agent-evals | Agent quality includes tool choice, traces, policy following, and workflow behavior. | Versioned fixtures and deterministic route/guard tests provide regression evidence beyond prose. | Important obvault-backed answers should become traceable durable notes only when sourced. |
