# Workflow Scaffold Best Practices Audit

Date: 2026-06-12

Repo: `/Volumes/Crucial/work/etabli`

Objectif: comparer le workflow scaffold Etabli aux pratiques recentes observees chez Anthropic, OpenAI, GitHub, Kiro, LangChain et Spec Kit, puis lister ce qu'il faut garder, modifier, ajouter ou eviter/supprimer.

Non-objectifs: ce document ne modifie pas le workflow scaffold. Il ne remplace pas un plan d'implementation et ne cree pas de ticket.
...
- `workflow-scaffold/templates/docs/agent-memory.md`
- `workflow-scaffold/templates/docs/plan.md`
- `workflow-scaffold/templates/docs/claude-code-workflow.md`
- `workflow-scaffold/templates/docs/project-context.md`
...
## Etat actuel du workflow scaffold Etabli
...
- `scripts/scaffold-project` expose `--new`, `--convert`, `--dry-run`, `--force` et appelle `scripts/deploy-workflow`.
- `scripts/deploy-workflow` detecte les conflits, sauvegarde avant overwrite avec `--force`, et ajoute `PLAN.md` a `.gitignore`.
- `workflow-scaffold/templates/docs/agent-workflow.md` ne liste pas toute la surface aujourd'hui deployee par `scripts/deploy-workflow`: `workflow/memory.md`, `workflow/plan-archive.md`, `docs/agent-memory/README.md`, `docs/plan/README.md`.
- `workflow-scaffold/templates/docs/claude-code-workflow.md` contient deja une doctrine proche de l'article Anthropic: workflow simple par defaut, planner/builder/evaluator seulement quand utile, contrats de sprint, handoff, evaluation UI.
...
- Le workflow scaffold est destine a etre portable entre projets, pas seulement a ce repo.
...
| A garder | `confirmed` | Garder `AGENTS.md` et `CLAUDE.md` comme adaptateurs courts vers des docs suivies. | OpenAI, GitHub et AGENTS.md convergent sur des instructions repo lisibles, proches du code, avec hierarchie de contexte. | Instructions longues, stale, ou chargees par trop d'outils differents sans priorite claire. | Aucun. | `workflow-scaffold/templates/AGENTS.md`, `workflow-scaffold/templates/CLAUDE.md` |
| A modifier | `confirmed` | Mettre a jour `docs/agent-workflow.md` pour lister exactement la surface deployee. | Le script deploye plus de fichiers que la doc installee ne mentionne. C'est une derive de source de verite deja observable. | Un agent ou un humain croit que la memoire/les archives ne font pas partie du workflow scaffold installe. | Faible. | `workflow-scaffold/templates/docs/agent-workflow.md`, `tests/workflow-scaffold-smoke.sh` |
| A modifier | `confirmed` | Ajouter une taxonomie courte des artefacts d'etat: `PLAN.md`, `docs/plan/`, `docs/agent-memory/`, `docs/agent-runs/`, tickets. | Les sources context-engineering distinguent contexte actif, memoire, handoff et evaluation. Etabli a les pieces, mais leur relation est dispersee. | Mauvais artefact au mauvais moment: archives creees trop tot, memoire transformee en logs, plans anciens reutilises comme specs vivantes. | Faible a moyen. | `workflow/spec.md`, `workflow/memory.md`, `workflow/plan-archive.md`, `workflow-scaffold/templates/docs/agent-workflow.md` |
| A ajouter | `confirmed` | Ajouter une petite suite de regression du workflow scaffold. | OpenAI et Anthropic traitent les workflows agentiques comme des systemes a evaluer: traces, evals, regression tasks, resultats reproductibles. Etabli a des smoke tests, mais pas de banque de comportements workflow critiques. | Les regles regressent silencieusement: plan archive trop tot, ticket template devie, conversion ecrase des fichiers, review oublie le plan. | Moyen. | `tests/`, `workflow/workflow-scaffold-regression.md` ou doc equivalente |
| A modifier | `confirmed` | Rendre explicites les criteres d'escalade vers evaluateur frais/subagent. | Anthropic montre que planner/generator/evaluator aide surtout pour les taches longues, subjectives ou aux limites du modele; sinon c'est du cout. Etabli dit deja "seulement quand utile", mais les triggers peuvent etre plus operationnels. | Sous-evaluation des gros changements ou, inversement, usage reflexe de multi-agent lourd. | Faible. | `workflow/spec.md`, `workflow/review-rubric.md`, `workflow-scaffold/templates/docs/claude-code-workflow.md` |
| A modifier | `confirmed` | Formaliser une discipline de contexte: quoi lire, quoi resumer, quoi compacter, quoi sortir du contexte. | Anthropic Cookbook et LangChain decrivent la memoire, la compression, l'isolation et le nettoyage des retours d'outils comme leviers distincts. Etabli a memoire/archive, mais pas encore une politique courte et commune. | Context rot, repetition de gros dumps, agents qui se basent sur une conversation plutot que sur les artefacts suivis. | Moyen-faible. | `workflow/memory.md`, `workflow-scaffold/templates/docs/claude-code-workflow.md`, `workflow-scaffold/templates/AGENTS.md` |
| A ajouter | `proxy-supported` | Ajouter une mini doctrine de design des tools/scripts du workflow scaffold. | Anthropic insiste sur des tools a but distinct, faible bruit, noms clairs, sorties actionnables. Etabli ajoute des scripts (`deploy-workflow`, `scaffold-project`, audits); leur design merite une checklist avant expansion. | Scripts redondants, sorties trop verbeuses, options ambiguës, faible testabilite. | Faible a moyen. | `workflow/tool-design.md` ou section dans `docs/agent-workflow.md`, `scripts/` |
| A modifier | `confirmed` | Clarifier `scaffold-project` comme commande user-facing et `deploy-workflow` comme primitive deploiement. | Le README le dit deja, mais montre encore les deux dans le quick path. L'utilisateur doit avoir une porte d'entree nette. | Choix inutile entre deux commandes, adoption plus fragile dans projets existants. | Faible. | `README.md`, `workflow-scaffold/templates/docs/agent-workflow.md` |
| A ajouter | `proxy-supported` | Ajouter un mode `--check` ou un rapport machine-readable optionnel pour conversion de projets existants. | Spec Kit supporte l'initialisation dans un projet existant; Etabli a dry-run et conflits, mais pas de sortie structuree exploitable par CI/agent. | Conversion manuelle peu auditable, difficile de detecter la derive du workflow scaffold dans un repo deja equipe. | Moyen. | `scripts/deploy-workflow`, `scripts/scaffold-project`, `tests/workflow-scaffold-smoke.sh` |
| A garder | `confirmed` | Garder l'archive de plan comme memoire distillee, pas comme copie brute. | Les sources de context engineering poussent a conserver du signal reutilisable, pas des logs exhaustifs. Etabli encode deja decisions, drift accepte, validation, follow-up. | Archives longues, peu lues, et donc inutiles comme memoire projet. | Aucun. | `workflow/plan-archive.md`, `workflow-scaffold/templates/docs/plan.md` |
| A modifier | `confirmed` | Ajouter dans le format d'archive une ligne "Superseded source-of-truth docs/specs" obligatoire quand un plan a fait diverger une spec. | La demande initiale vise les specs qui driftent. Le template l'a deja sous "Follow-up State", mais ce point devrait etre assez visible pour forcer l'agent a dire ce qui est devenu faux. | La memoire dit ce qui a ete fait, mais pas quelle ancienne spec ne doit plus etre crue. | Faible. | `workflow/plan-archive.md`, `workflow-scaffold/templates/docs/plan.md` |
...
| A supprimer / deplacer | `approximate` | Retirer les references trop centrales a `docs/agent-runs/` du chemin par defaut, ou scaffold un README correspondant. | `docs/claude-code-workflow.md` recommande `docs/agent-runs/<slug>/contract.md` et `handoff.md`, mais `deploy-workflow` ne cree pas de doc racine pour cette surface. Ce n'est pas bloquant, mais c'est un espace d'etat implicite. | Des handoffs apparaissent hors taxonomie, puis concurrencent `PLAN.md`, `docs/plan/` ou `docs/agent-memory/`. | Faible. | `workflow-scaffold/templates/docs/claude-code-workflow.md`, eventuel `workflow-scaffold/templates/docs/agent-runs.md` |
...
| A ajouter | `confirmed` | Ajouter une regle de recherche web: sources recentes, primaires si possible, et labels d'incertitude pour claims. | Les instructions projet le demandent deja; le workflow scaffold portable ne l'explicite pas fortement. Pour un workflow scaffold qui produit decisions et memoire, la fraicheur et la provenance font partie du contrat. | Rapports perimes, recommandations "best practice" non sourcees. | Faible. | `workflow-scaffold/templates/AGENTS.md`, `workflow-scaffold/templates/docs/agent-workflow.md` |
...
Modification utile: la carte `workflow-scaffold/templates/docs/agent-workflow.md` doit correspondre au deploiement reel. Sinon le workflow scaffold enseigne sa propre version stale.
...
Etabli encode deja cette prudence dans `docs/claude-code-workflow.md`. Le point faible est operationnel: quand exactement escalader?
...
### 4. Evals et regression du workflow scaffold

OpenAI et Anthropic traitent l'agent et son environnement comme un systeme mesurable: traces, evals, regression tasks, graders, retour humain ou modele. Etabli a des tests smoke, mais il manque une definition des comportements critiques du workflow scaffold.
...
- `deploy-workflow --force` cree des backups;
...
Decision: ajouter une petite suite de regression du workflow scaffold, pas un framework de regression lourd.
...
Anthropic recommande des tools clairs, distincts, peu bruyants, faciles a evaluer. `scripts/scaffold-project` et `scripts/deploy-workflow` vont dans la bonne direction:
...
Amelioration utile: un mode `--check` ou `--json` permettrait aux agents/CI de detecter une derive du workflow scaffold sans parser du texte. Ce n'est pas urgent, mais c'est cohérent avec un workflow scaffold qui se veut deployable et verifiable.

## Priorite proposee

1. Corriger `workflow-scaffold/templates/docs/agent-workflow.md` pour refleter la surface deployee.
2. Ajouter une taxonomie courte des artefacts d'etat dans `workflow/spec.md` ou `docs/agent-workflow.md`.
3. Renforcer `workflow/plan-archive.md` avec le champ visible "Superseded source-of-truth docs/specs".
4. Ajouter une mini suite de regression du workflow scaffold, probablement dans `tests/` plus une doc courte.
5. Clarifier les triggers d'evaluateur frais dans `workflow/review-rubric.md` et `docs/claude-code-workflow.md`.
6. Ajouter une checklist tool/script design avant d'etendre les commandes.
7. Envisager `scaffold-project --check` ou `deploy-workflow --json` apres stabilisation des tests.
...
- `unknown`: je n'ai pas de donnees d'usage reelles sur la frequence des erreurs de workflow scaffold dans plusieurs projets deployes. Les priorites sont donc basees sur inspection locale et sources externes, pas sur telemetrie.
- `approximate`: Kiro est utilise comme comparateur produit public; ses details internes de workflow ne sont pas verifiables depuis les sources publiques consultees.
- `proxy-supported`: la recommandation `--check`/`--json` vient de la logique d'evals et d'automation, pas d'une source disant explicitement que les workflow scaffolds doivent exposer cette option.
- `blocked`: je n'ai pas acces aux donnees internes de performance des workflows agentiques Anthropic/OpenAI/Kiro/GitHub; les conclusions utilisent leurs publications publiques, pas leurs traces internes.
- `confirmed`: la derive de `docs/agent-workflow.md` est locale et observable.
