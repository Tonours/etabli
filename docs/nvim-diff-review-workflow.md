...
This keeps the flow safe and explicit while removing the manual paste step: the prompt is still visible in the scratch preview and copied to registers, but the CLI also starts with the diff prompt already injected. Short prompts are passed as the first CLI message. Large batch prompts avoid OS argument-length failures by opening the CLI without a prompt argument and queueing the prompt through terminal paste input.
