...
- `<leader>ra` add a review comment on the current line in an active Hunk session; if no session exists, it opens Hunk and asks you to rerun the annotation
- visual `<leader>ra` add a review comment on the selected line range in Hunk, with Hunk anchoring to the first selected changed line
- `<leader>rs` runs a manual Hunk note persistence checkpoint
...
Use `:ReviewStart` before annotating when you opt into legacy GitHub-style draft review behavior. In the default Hunk flow, `:ReviewAnnotate` writes only to an active Hunk session; when no session exists, it opens Hunk and asks you to rerun the annotation instead of writing to local review state. Hunk notes are mirrored locally best-effort after direct comments, before reloads, when Hunk terminal buffers close, before Neovim exits, and after provider sessions. Run `:ReviewHunkSync pull` as a manual checkpoint before risky exits. When legacy commands are enabled and a local transaction is active, `:ReviewLegacyAnnotate` stores pending comments in the local transaction instead of immediately adding submitted comments. `:ReviewPreview` shows the pending review, `:ReviewExport markdown|json` opens an export buffer, and `:ReviewSubmit comment|approve|request-changes` commits the pending comments into local review state. Submission refuses stale draft hunks when the diff changed before submit.
...
Etabli auto-persists live Hunk notes into local review state before active-session reloads, after direct comments, on Hunk terminal close, on `VimLeavePre`, and after Claude/Pi provider sessions. It auto-rehydrates unresolved local comments and open agent findings into active Hunk sessions after reload/open when a session is visible. Use `:ReviewHunkSync` or `:ReviewHunkSync pull` as an explicit persistence checkpoint. Use `:ReviewHunkSync push` to force rehydration into the active Hunk session. Use `:ReviewHunkSync both` only when you intentionally want to pull first, then push local unresolved notes back into Hunk. Pushed comments carry an `Etabli id` marker so repeated syncs can skip duplicates.
...
Claude/Pi review commands require a live Hunk session. If none exists, Etabli opens `hunk diff --watch` and asks you to rerun the provider command after the session is ready. Provider prompts are dispatched only after an active Hunk session exists; when a session already exists, Etabli persists live notes, reloads Hunk, rehydrates local notes, and then dispatches the interactive provider prompt.

The Hunk review action is intentionally read-only. It asks the provider to inspect the live session with `hunk session review --repo <repo> --json`, add inline notes with `hunk session comment apply --stdin --json` or `comment add`, and end with `GO`, `GO WITH NOTES`, or `BLOCK`. It does not ask the provider to edit files or write findings into local review state. The default Hunk flow does not parse agent stdout; findings must become Hunk inline comments through the session commands in the prompt, then Etabli mirrors those live Hunk notes locally after the provider exits.

Legacy agent ingest is available only when `vim.g.etabli_review_legacy_commands = 1`. `:ReviewIngestClaude [file]` and `:ReviewIngestPi [file]` import structured output into local review state instead of Hunk. Without a file argument, the command reads the unnamed register. Imported findings must use the structured labels requested by the legacy review prompt: `severity:`, `file:`, `line:` or `line_range:`, `issue:`, `impact:`, `review_comment:`, and optional `suggested_fix:`. Findings are anchored to live hunks before being stored; unmatched or duplicate findings are skipped. The legacy inbox shows provider counts such as `C:1` and `P:2`, legacy inline annotations show compact agent markers, and `<leader>rg` or `:ReviewCompareAgents` compares Pi and Claude findings for the current hunk only when legacy keymaps or commands are enabled.

Legacy suggested changes from imported `suggested_fix:` fields are preview-first. Use `<leader>rS` or `:ReviewSuggestionPreview` only after enabling legacy keymaps or commands. The preview opens markdown with the original finding, suggested fix, and a safety classification. Text-only suggestions are `preview-only`; diff suggestions are marked `safe-preview` only when all file markers target the current review file, otherwise they are `unsafe`. This flow does not auto-apply arbitrary agent patches. Use `:ReviewSuggestionStatus applied|rejected|resolved|open` to track the decision after manual review or application.
...
Hunk is the default visible review UI and default command surface. Default commands and keymaps route through `config.review.hunk_flow`, a small Hunk orchestrator that opens/reloads sessions, focuses the current line, and launches HITL prompts. Opening a new Hunk inbox resolves the Git repo directly and does not load the legacy `config.review` UI. When an active Hunk session exists, the invisible local persistence adapter may load `config.review.state` and `config.review.items` to mirror and rehydrate notes; `config.review.providers` loads only for provider review commands. Local persistence, multiline comment editing, note import/export, and cache refresh live behind `config.review.hunk_local_adapter`, which is loaded on demand by Hunk session lifecycle, explicit persistence, or legacy fallback paths. The larger `config.review` legacy module is not loaded during default startup unless legacy flags are enabled.

Local review state remains as a persistence adapter only for capabilities Hunk does not yet persist after session close: statuses, draft transactions, stale review markers, suggested-fix status, exact multiline range anchors, automatic note mirroring, and explicit note rehydration through `:ReviewHunkSync`.
