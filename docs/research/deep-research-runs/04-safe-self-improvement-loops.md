...
- [S19] "workflow-event-detail.jq harness_validation_completed accepted gate" — "/Users/tonours/work/etabli/scripts/lib/workflow-event-detail.jq" (independently checked against "workflow-event-detail.jq; events.md; -suite.mjs compareStrategies" — "/Users/tonours/work/etabli/scripts/lib/workflow-event-detail.jq")
...
- "Question 2 uncertainty: Inspectable baseline/candidate run JSON under `workflow//results/` (e.g. baseline-run.json, candidate comparison records) is the  suite’s concrete run-record convention, not a universal mandatory path named in the harness_validation_completed schema."
...
- "Question 4 uncertainty:  live pass@1/pass^3 under paid budget is a separate measurement plane; residual-risks notes candidate reject is offline comparative only when live budget is zero."
