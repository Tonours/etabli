...
**Périmètre :** Pi + Claude, ledgers, évaluations vNext, et boucle locale de self-improvement.
...
4. **« sealed held-out » n'implique pas confidentialité.** Les 13 cas sealed held-out sont versionnés et lisibles dans `workflow/vnext/tasks.json`; le hash de corpus fige le contenu mais ne l'isole pas du candidat. Source : `workflow/vnext/tasks.json`, `tests/vnext-suite-smoke.sh`.
...
| Élément | Contenu |
| --- | --- |
| **Problème confirmé** | `harness_validation_completed` accepte des noms de population et de candidat arbitraires, y compris une comparaison 0/1 → 1/1 ; les cas held-out vNext sont publics dans le dépôt. |
...
- `workflow/vnext/tasks.json`, `scripts/lib/vnext-suite.mjs`
