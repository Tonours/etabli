...
## Outcome
Work `bug-check` / `pr-qa` / `sec-pr` are thin pointers. Forest Dependabot extras live in `claude/scopes/work/skills/sec-pr/references/forest-dependabot.md`. Ten unused Claude slash commands are gone; `/plan` is no longer an alias of plan-create. `bff-ticket-loop` stays in work scope. Adapter Source resolution 5-path lists are replaced by a one-line home fallback (plan-loop keeps its embedded PLAN template).

## Context
- Follow-up of `docs/plan/20260819-skill-default-load.md`
- `bff-ticket-loop` is a Forest BFF epic ticket factory, not Etabli kernel
...
- Rationale: it is still the PRD-637-style Forest BFF decomposer; work-scoped, not default-loaded
- Consequences: Forest epic ticket loop remains available on work machines
