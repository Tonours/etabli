...
- command: `--status` across all `~/work/*/PLAN.md`
  - result: 7 flagged, 4 clean. Stale: `employer` 138d, `employer-poc` 47d.
    Router-invisible: `claude-repo` (`READY — all 6 slices IMPLEMENTED + validated`),
    `employer-bugfixes` (`DONE`), and three plans with no `Status:` line
    (`Gc-feature-workflow-editor-palet`, `employer-express-sequelize`, `pi-read`).
