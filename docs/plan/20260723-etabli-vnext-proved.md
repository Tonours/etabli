# Implemented plan: Etabli  outcome suite (deterministic)

**Date:** 2026-07-23  
**Status:** **done** (deterministic  package); live effectiveness **waived** by user 2026-07-23  
...
| Path | Change |
| --- | --- |
| `workflow//population.json` | Freeze metadata for `etabli--initial-v1` |
| `workflow//tasks.json` | 32 tasks, 12 sealed held-out (37.5%), multi-category coverage |
| `scripts/-suite` | Entry point |
| `scripts/lib/-suite.mjs` | Drivers, pure graders, trial schema, metrics, compare |
| `tests/-suite-smoke.sh` | Inventory + suite + meta invariant + security metrics |
| `workflow/runtime/agentic-infra-checks.tsv` | `-suite-smoke` |
...
| Check | Result |
| --- | --- |
| `scripts/-suite --json` | 32/32 pass@1=1.0 |
| `bash tests/-suite-smoke.sh` | ok |
...
`.workflow/etabli--proved/events.jsonl` (gitignored local)

## Inspectable results (durable in-repo)

- `workflow//results/baseline-summary.json`
- `workflow//results/baseline-run.json`
- `workflow//results/candidate-1-reward-hack.json`
- `workflow//results/live-blocked.json` (mechanical live gate)
- `workflow//results/goal-completion.json` (done + live waiver)
- `workflow//results/residual-risks.md`

## Post-archive hardenings (same goal window)

- `scripts/-suite --live-status` + `liveBudgetStatus()`: mechanical live budget gate (unset/0 → blocked; positive → can_run_live).
- `tests/-suite-smoke.sh` asserts live gate honesty and durable `workflow//results/live-blocked.json`.
- Durable inspectable results under `workflow//results/` (baseline, candidate reject, live block, residual risks).
...
- `workflow//results/live-blocked.json` (mechanical live gate)
- `workflow//results/goal-completion.json` (done + live waiver) records **waived**, not silent success.
