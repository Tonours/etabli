...
- Four Forest/personal skills that lived only in `~/.claude/skills/` as real
...
- New `forest-backend-suite` router skill covers the half of the knowledge base
...
- `~/work/brain/kb`: 58 notes — bff 11, zendesk 8, forest 7, mfe 6, agent 5,
...
- Choice: a single `forest-backend-suite`.
- Rejected options: five separate skills, one per domain.
- Rationale: `ember-forestadmin-suite` proves the router pattern works, and one
...
- command: pointer sweep over `forest-backend-suite/SKILL.md`
