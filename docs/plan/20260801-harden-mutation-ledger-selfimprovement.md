...
- `workflow/self-improvement/manifests/core-v1.json` (new): declares
  ...
  (`etabli-vnext-initial-v1`) with corpus hashes, the evaluator
  (`scripts/vnext-suite`) hash, a minimum sample per split, and
...
- Focused smokes: `plan-cleanup`, `no-progress-mutate-deny` (corrupt/post-terminal/
  ...
  (`&&`/`||` read-only with fail-closed on mixed commands), `vnext-suite`,
