...
## Accepted Drift
- Original plan/spec: 30 employer+Etabli PRs with review-comment gold; judge `claude-haiku-4-5`.
...
- Why accepted: employer crawl not required for v1 structure; haiku not in `enabledModels`; unlabeled human rows still exercise ingest/split/score plumbing.
...
## Follow-up State
- Remaining risks: human gold unlabeled (vacuous pass); heuristic ≠ real reviewer; no employer PRs; self-family judge if LLM path used.
