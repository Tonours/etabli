...
- `scripts/-suite --inventory`: 35 tâches, 13 sealed held-out, fraction 0,3714.
- `workflow//results/baseline-summary.json`: corpus offline enregistré 35/35; utilisé comme gate de régression, pas comme benchmark avec headroom.
...
- Plan adversary initial: `GO WITH NOTES`; trois findings acceptés — total tokens de tous les participants, seuil anti-hallucination,  comme gate de régression.
...
- `scripts/verify-agentic-infra core`
  - result: passed; 224 Pi tests, router-eval 53/53, /guards/contracts smokes green.
