...
| `workflow//population.json` + `tasks.json` | Corpus d'évaluation | Fige le corpus et cible une fraction held-out | Tous les cas, y compris 13 `sealed:true`, sont trackés/lisibles : **frozen public**, pas held-out secret. |
| `scripts/lib/-suite.mjs` | Grader final offline | Exécute drivers/grader déterministes et hash le corpus | Un candidat dans le même worktree peut lire/modifier les graders sans une séparation de promotion. |
...
user intent
  ...
  → evaluator  (frozen public corpus)
