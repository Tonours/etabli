...
Cross-cutting:
  ...
  workflow-metrics / retrospect / dossier / monitor  (read-only)
   suite (scripts/-suite)  final-state graders
...
|  eval | suite tasks | trials/graders | pass@1 splits | `scripts/-suite` 35 tasks (**confirmed**) |
...
| **Held-out / grader semantics ( + harness candidates)** | **confirmed** when suite used | harness_validation_completed schema;  meta-run-finished-not-success; sealed held-out fraction | `workflow/events.md:86-91`; `scripts/lib/-suite.mjs`; inventory sealed≈0.37 |
...
| Large dirty worktree | Uncommitted /graph/project-autonomy/kimi — vérité opérationnelle ≠ HEAD commit | **confirmed** `git status` baseline |
...
| Live effectiveness | Explicitly waived / opt-in elsewhere; not part of default green suite | **confirmed**  research/live-blocked artifacts |
...
|  suite | oui | oui | n/a | oui | n/a | **confirmed** | `scripts/-suite`; inventory 35 tasks sealed≈0.37; meta-run-finished-not-success in `scripts/lib/-suite.mjs` |
...
**Synthèse scorecard:** les boucles de **routing / permissions /  / retrospect / self-improvement design** sont les plus solides (**confirmed**). Les boucles **host-dependent** (`/goal`, live ci-fix end-to-end) restent **partiel** ou **approximate** faute de preuve live dans cette session.
...
|  / meta-run-finished | forbids run-finished-as-success | n/a | **confirmed** grader invariant |
| outcome_metric history | may treat “completed run” as success | n/a | **approximate** semantic weak for productivity |

**Reward-hacking residual:** narrow smokes + docs-smoke string pins can be gamed without improving user outcomes (**approximate**). Mitigation path exists ( graders, held-out, adversary) but daily usage still optional.
...
| `scripts/-suite` (35 tasks) | Deterministic host/control-plane outcomes + graders | Multi-turn paid agent reliability (live opt-in / waived) | **confirmed** |
...
- **Surface:** extend  population carefully with real failures
...
| `git status --short` | Dirty worktree: /graph/project-autonomy/kimi uncommitted + capability/docs mods | **confirmed** |
| `scripts/verify-agentic-infra all` | **EXIT:0** (includes router-eval accuracy=1, alignment=1, ops_stop_misses=0) | **confirmed** |
| `scripts/-suite --inventory` | ok, 35 tasks, sealed held-out ≈0.37 | **confirmed** |
...
Etabli est déjà un **control plane evidence-first** mature: routing partagé, READY/ops-stop, ledgers typés, self-improvement sans auto-apply, answer-quality floor,  graders, project-autonomy envelope, suite canonique **verte** sur le HEAD observé (**confirmed**).
