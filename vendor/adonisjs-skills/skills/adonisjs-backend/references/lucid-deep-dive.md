...
AdonisJS 7 is migrations-first. After migrations run, Lucid scans the database and generates strongly typed schema classes (`#database/schema`):

- models extend the generated schema class and inherit column definitions
- stop redeclaring columns with `@column` — the migration is the single source of truth
- schema classes regenerate when migrations run, keeping models in sync with the real schema
- for legacy databases, generate schema classes directly from the DB without recreating migration history
...
- use the v7 Transformer layer (`BaseTransformer` / `toObject`, exposed via the `serialize()` helper; variants and `whenLoaded` for intentional shapes) to own the response shape — models stay persistence-only
