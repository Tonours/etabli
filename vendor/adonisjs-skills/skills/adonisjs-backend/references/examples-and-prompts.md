...
Move slow work out of the request in AdonisJS 7:
- dispatch a typed `Job<T>` through the first-party experimental `@adonisjs/queue`,
- keep the controller thin and the trigger point explicit,
- assert dispatch with the queue fake (`QueueManager.fake()` + `assertPushed`).
