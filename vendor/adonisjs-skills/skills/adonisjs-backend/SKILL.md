...
- validation: Vine
- persistence: Lucid ORM (migrations-first schema classes), relationships, scopes, migrations, seeders, query builder, transactions
- email: Mail
- auth: official Adonis auth package and guards
- authorization: Bouncer or the existing official authorization path
- serialization: Transformer layer (`@adonisjs/core/transformers`) for response contracts
- rate limiting: `@adonisjs/limiter` on sensitive endpoints
