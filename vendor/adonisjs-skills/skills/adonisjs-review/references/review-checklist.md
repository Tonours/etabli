...
- Do models extend generated schema classes instead of redeclaring columns with `@column`?
- Is raw SQL used only where justified?
- Are migrations focused and reversible?
- Is response serialization transformer-backed and safe relative to model internals?
...
- Are abuse-prone endpoints (login, signup, reset) rate-limited via `@adonisjs/limiter` (weighted when cost varies)?
- Are unauthorized redirects using intended-URL helpers rather than open/custom bounce-back URLs?
- Are v7 security defaults kept (UUID upload rename, `$`-key mass-assignment protection, redirect host validation)?

## 5. Mail and framework services

- Is Mail used instead of ad hoc SMTP wrappers?
- Are framework services configured centrally?
- Are providers and `adonisrc.ts` hooks used for boot wiring rather than random imports?
- Is side-effect timing understandable?
- Where flow matters, is `@adonisjs/otel` tracing used instead of ad hoc logging?

## 6. Commands, events, scheduler, and queues
