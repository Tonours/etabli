...
- Vine validation
- Lucid usage (schema classes, no `@column` redeclaration)
- provider and `adonisrc.ts` hook wiring
- events/commands placement
- background jobs on the first-party experimental `@adonisjs/queue`
- rate limiting on sensitive endpoints (`@adonisjs/limiter`)
- response serialization through Transformers
- test coverage
