...
6. Check `.safe()`, `isStatus()`, or `isValidationError()` handling for typed failure paths.
7. Add tests for contract-sensitive changes.
8. Run the Tuyau safety harness before finalizing.
...
- `@tuyau/react-query` / `@tuyau/vue-query` for TanStack Query; `@tuyau/superjson` when rich types must cross HTTP,
