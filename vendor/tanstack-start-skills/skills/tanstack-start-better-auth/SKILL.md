...
6. Prefer the official Better Auth + Start CLI scaffold when starting greenfield (`better-auth` docs document a Start-integrated create flow); keep the same handler/cookie/session boundaries if you wire manually.
...
Prefer the helper Better Auth documents for TanStack Start: `getRequestHeaders()` from `@tanstack/react-start/server` (plural). Current Start server APIs also expose `getRequest()` / `getRequestHeader()`; either path is valid if you pass the request headers into Better Auth.
