...
TanStack Start builds with either Vite or Rsbuild. Most provider guides assume Vite; verify Rsbuild compatibility for your target host before committing to it.
...
## Environment rules
- client-safe build-time values use the build tool's public prefix: `VITE_*` (Vite) or `PUBLIC_*` (Rsbuild default)
- secrets stay unprefixed and server-only — never under `VITE_*` or `PUBLIC_*`
