...
**Data/API boundary first.** Official Start separates:
- **data/API boundary** (security): every server function, server route, or API endpoint that touches private data must authorize the request itself
- **route/UI layer** (UX): `beforeLoad` keeps users out of screens they cannot use and avoids failed work

Pair DIY session/CSRF/rate-limit work with the official **Authentication Server Primitives** guide. Managed providers (Clerk, WorkOS, Better Auth, etc.) still need the same boundary rule.
...
- `beforeLoad` alone is a data authorization boundary
...
- client-safe config only via the build tool's public prefix (`VITE_*` for Vite, `PUBLIC_*` for Rsbuild by default)
- secrets stay server-only in `process.env` (unprefixed)
- never expose secret keys via `VITE_*` or `PUBLIC_*`
