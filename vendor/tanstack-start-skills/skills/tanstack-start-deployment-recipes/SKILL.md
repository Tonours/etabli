...
- your build tool (Vite or Rsbuild) is supported by the chosen host's adapter; most documented recipes assume Vite
...
### Recipe G — Appwrite Sites
Choose when:
- the team already standardizes on Appwrite Sites
- managed git-connected deploys are preferred

High-level actions:
1. push the Start app to a GitHub repository
2. create/select an Appwrite project and Sites target
3. connect the repository and confirm framework detection as TanStack Start
4. verify build settings (install/build commands; output `./dist`, or `./.output` when using Nitro v2/v3)
5. set required environment variables and deploy
6. visit the site and validate SSR/server functions

Primary files/surfaces to inspect:
- `package.json` build scripts
- Appwrite Sites env and build settings

Validation focus:
- framework detection and output directory match the actual build tool
- secrets are present only as server env
- public routes and server functions work after deploy
