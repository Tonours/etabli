---
name: ember-forestadmin-lint-style
description: Fix and avoid ForestAdmin lint, typecheck, class naming, template, and SCSS convention failures.
version: 0.2.0
author: ForestAdmin Team
license: Proprietary
---

# Ember ForestAdmin Lint and Style
...
A custom ESLint rule (`forest-custom/class-naming-based-on-path`) auto-fixes a subset of class names to match file path and suffix. It errors on mismatch for the configured paths in `eslint.config.mjs`; use nearby files and `yarn run ember-generate` for paths not yet enforced.
...
React packages/MFEs use shared `@forest-admin/config` with strict ESLint 9 flat config:
- no `any` by default.
