...
/**
 * Memo of the last reconstruction. reconstructJsonlState is pure, and all
 * current callers treat the returned state as read-only (index.ts clones the
 * parts it keeps; compaction.ts only reads it), so an unchanged log — the
 * common case across session restores, compactions and dashboard exports —
 * skips the full JSON re-parse. String equality is O(1) for the same
 * reference and a fast memcmp otherwise.
 *
 * NOTE for future callers: do not mutate the returned object; clone first
 * (see cloneExperimentState / the mapping in reconstructState).
 */
let lastReconstruction: { content: string; state: ReconstructedJsonlState } | null = null;

export function reconstructJsonlState(jsonlContent: string): ReconstructedJsonlState {
  if (lastReconstruction !== null && lastReconstruction.content === jsonlContent) {
    return lastReconstruction.state;
  }
  ...
  lastReconstruction = { content: jsonlContent, state };
  return state;
}
