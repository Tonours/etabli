...
if [ "${RUN_AGENT_CLI_SMOKE:-}" != "1" ] && [ "${RUN_AGENT_CLI_SMOKE_SELF_TEST:-}" != "1" ]; then
  printf 'workflow CLI smoke test: skipped (set RUN_AGENT_CLI_SMOKE=1 to run real agent CLIs)\n'
  exit 0
fi
...
if [ "${RUN_AGENT_CLI_SMOKE_SELF_TEST:-}" = "1" ]; then
  ...
  printf 'workflow CLI bounded runner self-test: ok\n'
...
SH
  ...
  printf 'workflow CLI stub smoke: ok\n'
...
printf 'workflow CLI smoke test: ok\n'
