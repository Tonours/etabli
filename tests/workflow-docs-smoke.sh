...
assert_contains "$ROOT_DIR/docs/nvim-diff-review-workflow.md" 'persists live notes, reloads Hunk, rehydrates local notes'
...
assert_contains "$ROOT_DIR/docs/nvim-diff-review-workflow.md" 'loaded on demand by Hunk session lifecycle, explicit persistence, or legacy fallback paths'
assert_contains "$ROOT_DIR/docs/nvim-diff-review-workflow.md" 'auto-persists live Hunk notes into local review state'
