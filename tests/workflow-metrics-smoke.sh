...
"$ROOT_DIR/scripts/workflow-event" --dir "$EVENT_DIR" append run-a outcome_metric '{"outcome":"success","success":true,"measured":true,"input_tokens":10,"output_tokens":5,"total_tokens":15,"tool_calls":0,"elapsed_ms":50,"success_kind":"task_grader","grader_success":true,"participant_usage":[{"id":"parent","role":"parent","input_tokens":8,"output_tokens":4,"total_tokens":12},{"id":"reviewer","role":"reviewer","input_tokens":2,"output_tokens":1,"total_tokens":3}],"batch_wall_clock_ms":3600000,"batch_started_at":"2026-07-31T10:00:00Z","batch_terminal_at":"2026-07-31T11:00:00Z"}'
...
printf '%s\n' "$json_output" | jq -e '.totals.participant_breakdown_outcomes == 1' >/dev/null
printf '%s\n' "$json_output" | jq -e '.totals.batch_makespan_outcomes == 1 and .totals.batch_makespan_ms == 3600000' >/dev/null
printf '%s\n' "$json_output" | jq -e '.totals.verified_throughput_per_hour == 1' >/dev/null
...
case "$text_output" in
*tokens_per_successful_outcome=15*) ;;
*)
...
case "$text_output" in
*participant_breakdown_outcomes=1*) ;;
*)
	printf 'expected participant_breakdown_outcomes in text output\n%s\n' "$text_output" >&2
	exit 1
	;;
esac
case "$text_output" in
*verified_throughput_per_hour=1*) ;;
*)
	printf 'expected verified_throughput_per_hour in text output\n%s\n' "$text_output" >&2
	exit 1
	;;
esac
